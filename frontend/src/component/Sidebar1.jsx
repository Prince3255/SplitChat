    ...
    <Sidebar className="w-full hidden md:block border-r">
      <Sidebar.Items className="pt-4">
        <Sidebar.ItemGroup className="flex flex-col gap-6">
          {routes.map((route) => (
            <Sidebar.Item labelColor="dark" key={route.path} active={activeTab === route.tab} as="div" className={`cursor-pointer transition-all duration-200 ${activeTab === route.tab ? "bg-green-50 text-green-600" : "hover:bg-gray-50"}`}>
