...
//             <video ref={videoRef} autoPlay className="w-full h-full" muted></video>
//             <Button
//               onClick={() => {
//                 setMediaType(null);
//                 setCapturing(false);
...
function MessageInput({ setMessage }) {
  ...
  const handleVideoChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Prevent setting videoPreview or videoFile if recording is active
      if (mediaType === "video") {
        toast.error("Cannot upload video while recording is active");
        return;
      }
      setVideoFile(file);
      const reader = new FileReader();
      reader.onloadend = () => {
        console.log("Setting videoPreview from file upload:", reader.result);
